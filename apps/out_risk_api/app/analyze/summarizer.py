...
# 20260131 이종헌 신규: summary_ko/why/is_estimated 생성(LLM optional) + strict_grounding 규칙 적용
...
def esg_summarize_and_why(
    ...
    if _LC_AVAILABLE and os.getenv("OPENAI_API_KEY"):
        try:
            ...
            prompt = (
                "너는 외부 리스크 문서를 요약하는 분석기다.\n"
                "규칙:\n"
                "- 출처(입력 텍스트)에 없는 사실은 쓰지 말 것\n"
    ...
    summary = f"{category} 관련 외부 이슈 가능성. {snippet}"
    summary = esg_prefix_if_needed(strict_grounding, is_estimated, summary)
    why = snippet
